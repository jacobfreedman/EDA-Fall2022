...
```{r pull.and.plot.other.discharge.data}
theURL2 <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02085070&period=24&format=rdb'

#Get the data, which starts on line 30
gage_data2 <- read.table(theURL2,skip = 29,header=TRUE,sep='\t',stringsAsFactors = T) 
...
#Plot
gage_data_eno <- ggplot(gage_data2,aes(x=datetime,y=discharge_cfs)) + 
  geom_line() +
  labs(x = "Date",y = "Discharge (cfs)")
print(gage_data_eno)
```
...
```{r}
#Apply the API key
census_api_key("d4ad7874c996c7a83fec0a81466309ddc07d8e22", install = TRUE)
...
1. Navigate to the page. Note these data are for 2021
...
webpage <- read_html('https://www.eia.gov/electricity/state/')
...
the_price <- webpage %>% 
  ...
  html_text()
the_capacity <- webpage %>% 
  html_nodes("td:nth-child(3)") %>% 
  html_text()
net_generation <- webpage %>% 
  html_nodes("td:nth-child(4)") %>% 
  html_text()
total_retail <- webpage %>% 
  html_nodes("td:nth-child(5)") %>% 
  html_text()
  
#3 Construct a dataframe from the values

df_energy <- data.frame("Price" = as.numeric(the_price),
...
ggplot(df_energy, aes(x=State, y=Generation)) +
geom_point()
...
df_manyyears <- map(seq(2017,2019),scrape.it) %>% bind_rows()
