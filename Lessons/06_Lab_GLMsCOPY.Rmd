...
```{r}
#Exercise 1:
Ozone_density <- ggplot(EPAair) +
  geom_density(aes(x=Ozone, fill=as.factor(Year)), alpha=0.5)
  print(Ozone_density)
  
Ozone_boxplot <- ggplot(EPAair) +
  geom_boxplot(aes(x=as.factor(Year), y=Ozone))
  print(Ozone_boxplot)
...
Ozone_Regression <- lm(data=EPAair, PM2.5 ~ Ozone)
...
Ozone2.5 <- ggplot(EPAair, aes(x=Ozone, y=PM2.5)) +
  geom_point() +
  geom_smooth(method="lm", label=TRUE) +
  geom_text(x=120, y=20, label = "y=0.384x+15.638", hjust = 1)
  print(Ozone2.5)
...
EPANumeric <- 
  EPAair %>%
  select(meanLAT, meanLONG, Month, Year, PM2.5, Ozone) %>%
  na.omit()
  
EPAairCorr <- cor(EPANumeric)
...
AIC <- lm(data = EPANumeric, PM2.5 ~ Ozone + meanLAT + meanLONG + Month + Year)
summary(AIC)

step(AIC)
TPmodel <- lm(data = EPANumeric, PM2.5 ~ meanLAT + meanLONG + Month + Year)
summary(TPmodel)
...
RevisedRegression <- lm(data = EPANumeric, PM2.5 ~ Ozone + meanLAT + meanLONG + Month)
summary(RevisedRegression)
```
