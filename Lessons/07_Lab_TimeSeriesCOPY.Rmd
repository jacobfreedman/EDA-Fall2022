...
RainfallPlot <- ggplot(BRA_complete,aes(x=Date, y=Rainfall)) +
  geom_line() +
  geom_smooth(method = lm)
print(RainfallPlot)
  
TempPlot <- ggplot(BRA_complete,aes(x=Date, y=Temperature_C)) +
  geom_line() +
  geom_smooth(method = lm)
print(TempPlot)

#Rainfall looks constant, temp increases yikes!
...
Rainfall_ts <- ts(Rainfall_BR_processed$Rainfall, start = c(1901,1), frequency = 12)

Temp_ts <- ts(Temp_BR_processed$Temperature_C, start = c(1901,1), frequency = 12)
```
