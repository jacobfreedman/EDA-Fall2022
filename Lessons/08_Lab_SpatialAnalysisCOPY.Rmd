...
#yes, lat/long same as counties. FID,huc8,subbasin,acres,sqmiles,Hu8state,dwqbasin, and geometry. 
...
#Create a map
mapView(huc8_sf, col.regions = 'orange') + mapView(huc8_sf_filtered, col.regions = 'purple')
...
huc8_sf_wind <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', colClasses = c('HUC_12' = 'factor')) %>%
  mutate(huc8 = substr(HUC_12, 1, 8)) %>%
  group_by(huc8) %>%
...
huc8_sf_merged <- merge(x=huc8_sf, y=huc8_sf_wind, by.x='HUC_8', by.y='huc8')



#View the outputs
view(huc8_sf_merged)
mapView(huc8_sf_merged, zcol = 'kWhkm2day')
...
huc8_sf_aggregated <- huc8_sf %>%
  group_by(DWQ_Basin) %>%
  summarize(AcresTotal = sum(ACRES), SqMilesTotal = sum(SQ_MILES))
...
#Map the data
mapView(huc8_sf_aggregated, zcol = 'AcresTotal')
