...
upperNeuse <- st_read('./Data/Spatial/NCHUC8.shp') %>%
  filter(SUBBASIN == 'Upper Neuse')


#Create a map
mapview(huc8_sf, col.regions = 'red') +
  mapview(upperNeuse_sf, col.regions = 'blue')
...
intersecting_hucs <- huc8_sf_utm[the_triangle,]

mapview(intersecting_hucs) + mapview(the_triangle, alpha.regions = 0, color = 'orange')
#Intersect the HUC_8s

the_huc_clip <- huc2_utm %>%
  st_intersection(the_triangle)

mapview(the_huc_clip) + mapview(the_triangle, alpha.regions = 0, color = 'orange')
