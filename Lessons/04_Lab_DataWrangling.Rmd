...
NTL.phys.data.Tuesday.1990_1999 <- 
  NTL.phys.data %>%
  filter(year4 > 1989, year4 < 2000) %>%
  filter(lakename %in% c("Tuesday Lake"))
```
...
NTL.phys.data.Tuesday.1990_1999.July <- 
  NTL.phys.data %>%
  filter(year4 > 1989, year4 < 2000) %>%
  filter(lakename %in% c("Tuesday Lake")) %>%
...
mean(NTL.phys.data.Tuesday.1990_1999.July$temperature_C)
```
...
NTL.phys.data.gathered <-
  pivot_longer(NTL.phys.data, irradianceWater:irradianceDeck, names_to = "Location" , values_to = "Irradiance")
View(NTL.phys.data.gathered)

#Exercise 6: spread temperatureC into more than one column based on the depth
NTL.phys.data.spread <-
  pivot_wider(NTL.phys.data, names_from = "depth", values_from = "temperature_C")
```
