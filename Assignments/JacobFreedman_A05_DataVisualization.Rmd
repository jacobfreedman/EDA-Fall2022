...
NIWO_Litter <- read.csv("./Data/Processed/NEON_NIWO_Litter_mass_trap_Processed.csv", stringsAsFactors = TRUE)
...
Phosphates <- ggplot(PeterPaul_Processed) +
...
TPBoxplot <- ggplot(PeterPaul_Processed, aes(x=month,y=tp_ug)) +
  geom_boxplot(aes(color=lakename)) +
  scale_x_discrete(label = month.abb, drop = FALSE) +
  theme1 +
...
TNBoxplot <- ggplot(PeterPaul_Processed, aes(x=month,y=tn_ug)) +
  geom_boxplot(aes(color=lakename)) +
  scale_x_discrete(label = month.abb, drop = FALSE) +
  theme1 +
  theme(legend.position = "none")
...
> Answer: Total phosphorus and nitrogen appear to be larger on average in Peter Lake than Paul Lake regardless of season. However, there are large ranges for both variables and the results may not be significant (we would need to run T tests based on a time period of choice). Temperature, unsurpisingly, is warmest in the summer and coldest in thr spring and fall for both lakes. Temperature appears relatively similar for both lakes. 
...
NeedlePlot <- ggplot(NIWO_Litter_Needles) +
  geom_point(aes(x=collectDate, y=dryMass, color=nlcdClass))
  print(NeedlePlot)

#7
NeedlePlot2 <- ggplot(NIWO_Litter_Needles) +
  geom_point(aes(x=collectDate, y=dryMass)) +
