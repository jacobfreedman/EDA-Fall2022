...
```{r scrape.the.data}
#3
water.system.name <- DurhamWaterWebpage %>% 
  html_nodes("div+ table tr:nth-child(1) td:nth-child(2)") %>% 
  html_text()
water.system.name

pswid <- DurhamWaterWebpage %>% 
...
max.withdrawals.mgd <- DurhamWaterWebpage %>% 
  html_nodes("th~ td+ td") %>% 
  html_text()
max.withdrawals.mgd
...
df_max_withdrawals <- df_max_withdrawals %>% 
  mutate(Ownership = !!ownership,
         PSWID = !!pswid,
         Water_System = !!water.system.name,
...
Max_withdrawals <- The_website %>% html_nodes(Max_withdrawal_tag) %>% html_text()
...
Scraped_df <- Scrape_function(2018, '032-32-010')
...
Withdrawl_plot_comparison <- ggplot(Asheville_Durham_combined,aes(x=Date, y=Max_withdrawals_mgd)) +
  geom_line(color = ) +
  theme1 +
  ylab("Maximum Monthly Withdrawal")
print(Max_withdrawl_plot)
